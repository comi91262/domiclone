...
import (
	...
	domilike "github.com/comi91262/domilike"
...
func main() {
	...
	{
		logger = log.New(os.Stderr, "[domilike] ", log.Ltime)
	}
	...
	{
		cardSvc = domilike.NewCard(logger)
	}

	// Wrap the services in endpoints that can be invoked from other services
	// potentially running in different processes.
	var (
		cardEndpoints *card.Endpoints
	)
	{
		cardEndpoints = card.NewEndpoints(cardSvc)
	}
	...
	case "localhost":
		{
			...
			handleHTTPServer(ctx, u, cardEndpoints, &wg, errc, logger, *dbgF)
		}

		{
			...
			handleGRPCServer(ctx, u, cardEndpoints, &wg, errc, logger, *dbgF)
		}
